---
title: "Benchmarking the Doubleword Control Layer"
authors: [fergus]
tags: [ai-infrastructure, concepts, control-layer, benchmarking]
date: 2025-10-21
---
...
([dwctl](https://github.com/doublewordai/control-layer)) is the fastest AI
gateway around. We believe this because it's written in Rust[^1], and because we thought about performance a
...
It's so good that we are [open sourcing it](https://www.doubleword.ai/resources/doubleword-open-sources-the-worlds-fastest-ai-gateway). And once it's out there, it can be used in lots of
different places, in lots of different ways. And so, to prove that it will be
fast everywhere, we have to do benchmarking[^2].
...
The Doubleword Control Layer `dwctl` is an "AI Gateway". For the purposes of this discussion, this is
basically a [reverse
...
| [Tensorzero](https://github.com/tensorzero/tensorzero) | ✗    | ✓        | ✓    | No authentication story. _Super interesting project, with a lot to offer on the experimentation & evaluation side. Also written in Rust, and with strong benchmarks. I'd like to benchmark it anyway, but auth is a hard requirement for us, and auth vs. no auth is apples to oranges._                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| [Kong](https://portkey.ai/)                            | ✓    | ✓        | ✗    | Couldn't get it running _A product from a company that built enterprise gateways back before AI was all the rage. Getting their core gateway product up and running is really easy, and from what I know of it in that context, it's performant and nice to use. But I couldn't get the AI offering working in a reasonable amount of time. In order to make it an AI gateway I have to run some sort of shell piped bash script? And then that will transform my running regular Kong gateway into an AI gateway? Either way I tried it and it didn't work. I'd like to come back to this one though._                                                                                                                                                                                                                                                                                                                                                 |
| [Portkey](https://portkey.ai/)                         | ✓    | ✗        | ✗    | The hosted offering looks really [nice](https://portkey.ai/). But their OSS version is missing lots of features _They make a very visible claim that their product is open source, and as far as I can tell that's not accurate. They have an open source [offering](https://github.com/portkey-ai/gateway), but if you actually try to use it, you find that their [core features](https://github.com/portkey-ai/gateway?tab=readme-ov-file#core-features) list is a list of features that are available in their hosted product. And all the documentation links are to that product. From what I can tell from trying to use the OSS version, I can't actually use authentication. I gave them a cross for "easy to use" since it took me more than 30 minutes of looking to figure out that I couldn't do auth in the OSS product, which was very frustrating. If I'm wrong here, I'm happy to be corrected._ |
...
3. **[dwctl](https://github.com/doublewordai/dwctl)**: Rust, even more minimal (for now).
...
level enough that they're easy to forget about[^3].

[^3]: Another call to do your own benchmarking, where a system doing well because you made peculiar hardware choices is actually a good thing.
...
The source and the sink run together in the same binary[^4].
...
[^4]: Code [here](https://github.com/doublewordai/benchmark/tree/main/server)

### Hardware

We'll use pretty minimal hardware for a few reasons:

1. I suspect LiteLLM will be slower, for various reasons. Since it's written in
...
- **Total number of requests**: the total number of requests we work through in
  ...
  stability of the results, but not much else (as long as it's above 1.).
...
- **The amount of time the mocked sink waits between requests:** This lets us
...
Lets start with a simple one: 100 concurrent users, 1KB[^5] request sizes. The upstream server responds pretty
...
will return much less frequently)[^6].

[^5]: Pretty small, since most tokenizers are ~3 tokens per character, this is ~300 tokens.
[^6]: Really we should model both. But how many of each? This is why we should run our own benchmarks.
...
that's not accounted for in the wait time, and the network overhead[^7]

[^7]: In principle not all the overhead, since there's a network hop missing.

We can see that, at 100 concurrent users, dwctl keeps up very well. The overhead
stays in sight of the baseline, even at higher percentiles. Bifrost adds more
overhead than dwctl but still performs reasonably. LiteLLM on the other hand,
falls well behind - with over a full second of latency being added to the
median request.

| Metric           | Theoretical | Baseline | dwctl      | Bifrost | LiteLLM |
| :--------------- | :---------- | :------- | :--------- | :------ | :------ |
...
it's being hit from both sides - responses come back quick, and requests come
in quick. If you use a lot of fast models, this might be a realistic scenario.

The baseline starts to fall behind the theoretical numbers, and dwctl starts
...
latency. For both dwctl and Bifrost, this is the case - they're almost
indistinguishable from the baseline, with Bifrost performing slightly better on
throughput and average latency, while dwctl is better at P50 and P90. I'd
suspect we had some random blip here for dwctl that's increasing the tail
...
- **dwctl** performs best overall, with the lowest overhead across most
  scenarios. It keeps up well even under high load (100 concurrent users).
- **Bifrost** performs well at low load, matching dwctl's performance.
  However, it struggles significantly under high load, showing
  much higher latency than dwctl.
...
This isn't LLM inference optimization[^8],
where you're working on systems that are painfully slow, and where if they were
10 times faster you could change the whole world.

[^8]: If you care about that sort of stuff, see
[here](https://fergusfinn.com/blog/scheduling-in-inference-engines/),
[here](https://fergusfinn.com/blog/llm-guided-scheduling/),
[here](https://fergusfinn.com/blog/spacelike-speculative-decoding/),
[here](https://docs.doubleword.ai/conceptual/10-06-2025-bts-2)

The reason is that performance doesn't only matter when it's currently a
...
them, then they're doing a bad job. If API consumers notice that they're being
...
If you'd like to get started with the Doubleword Control Layer, join us on the
[github](https://github.com/doublewordai/control-layer). For issues, with our
performance or my benchmarking methodology, raise them on the [issues
page](https://github.com/doublewordai/control-layer/issues).
